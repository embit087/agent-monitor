#!/usr/bin/env bash
# Claude Code SessionStart hook for Notify Panel / winid registration.
# Cursor can import ~/.claude/settings.json as third-party hooks; when that happens,
# this script detects Cursor hook payloads and skips registration so Cursor's native
# sessionStart hook remains the single source of truth.
